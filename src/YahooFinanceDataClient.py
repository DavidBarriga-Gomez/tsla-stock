...
class YahooFinanceDataClient:
    ...
    def get_yahoo_finance_data(self, ticker, seq_history_len, seq_prediction_len, n_train_days):
        ...
        train = scaler.fit_transform(reframed_values[:n_train_days, :])
        test = scaler.transform(reframed_values[n_train_days:, :])
        ...
        dataModel = LSTMProcessedData(x_train, y_train, x_test, y_test, n_features, scaler)
        return dataModel
